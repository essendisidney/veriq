-- Partner snapshot logic lives outside the exposed API schema.
-- Thin public wrappers remain so the Next.js server can call them
-- with the service role. Anon and authenticated still have no EXECUTE.

create schema if not exists veriq;
revoke all on schema veriq from public, anon, authenticated;
grant usage on schema veriq to postgres, service_role;

create or replace function veriq.api_risk(p_token_hash text, p_company text)
returns jsonb
language plpgsql
stable
security definer
...
create or replace function veriq.api_snapshot(p_token_hash text, p_company text)
returns jsonb
language plpgsql
stable
security definer
...
revoke all on function veriq.api_risk(text, text) from public, anon, authenticated;
revoke all on function veriq.api_snapshot(text, text) from public, anon, authenticated;
grant execute on function veriq.api_risk(text, text) to postgres, service_role;
grant execute on function veriq.api_snapshot(text, text) to postgres, service_role;
...
stable
security definer
set search_path = veriq, public
as $$
  select veriq.api_risk(p_token_hash, p_company);
$$;
...
set search_path = veriq, public
as $$
  select veriq.api_snapshot(p_token_hash, p_company);
$$;
