-- Partner snapshot: API keys and share links can read a public pack
-- (score, findings, latest scan summary). No evidence file contents.

create index if not exists assets_share_link_hash_idx
...
revoke all on function public.veriq_api_snapshot(text, text) from public;
grant execute on function public.veriq_api_snapshot(text, text) to anon, authenticated;
